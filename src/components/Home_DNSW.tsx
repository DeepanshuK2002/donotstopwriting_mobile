...
const Home_DNSW: React.FC = () => {
  ...
  const handleStart = () => {
    ...
    // Fix: Using '../write/index.html' ensures we stay within the app's local filesystem context
    const base = isCapacitor ? '../write/index.html' : '/write';
    const url = `${base}?${tab === 'minutes' ? 'time' : 'words'}=${value}&hardcore=${isHardcore}`;
